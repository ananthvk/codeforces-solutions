...
int main()
{
    ...
    operations.resize(m + 1);
    vector<ll> cumulative(n+1, 0);
    vector<ll> count(m+1+1, 0);

    auto input_array = readvin(n);
    FORN(i, m)
    {
        // Read the operations into a map
        ll l = readn();
        ll r = readn();
        ll d = readn();
        operations[i + 1] = {l, r, d, 0};
    }
    FORN(i, k)
    {
        ...
        count[x] += 1;
        count[y+1] -= 1;
        //FORS(j, x, y + 1, 1) { operations[j].count += 1; }
    }
    // Apply the queries
    ll cuml_cnt = 0;
    FORN(i, m)
    {
        auto op = operations[i + 1];
        cuml_cnt += count[i+1];
        cumulative[op.l - 1] += cuml_cnt * op.d;
        cumulative[op.r] -= cuml_cnt * op.d;
    }
