...
ll match(std::string& s, ll index){
    if(index > s.size()){
        return 100;
    }
    if(index == s.size() - 1){
        // Check reverse
        if(s.size() >= 3){
            if(s[index] == '^' && s[index-1] == '_' && s[index-2] == '^')
                return 5;
        }
        if(s.size() >= 2){
    ...
    if(s[index] == '^'){
        ...
        if((index+1) < s.size() && s[index+1] == '_'){
            if((index+2) < s.size() && s[index+2] == '^'){
                return 2;
            }
            else{
                s.insert(s.begin() + index + 2, '^');
                return 2;
            }
        }
        else{
...
int main()
{
    ...
    FORN(tno, ntests){
        string s = reads();
        ll i = 0;
        ll operations = s.size();
        while(i < s.size()){
            i += match(s, i);
        }
        println(s.size() - operations);
    }
    return 0;
}
