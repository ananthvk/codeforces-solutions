...
std::string rightrotate(const std::string &s, ll k)
{
...
int main()
{
    ...
    FORN(nt, ntests)
    {
        string inp = reads();
        vl sum(inp.size(), 0);
        ll max_area = MINVAL(ll);
        vector<vector<long long int>> grid(inp.size());

        FORS(row, 0, inp.size(), 1)
        {
            string rot = rightrotate(inp, row);
            vector<long long int> ro;
            ENUMERATE(i, ch, rot, {
                ro.push_back(ch - '0');
                if (ch == '0')
                {
            ...
            ENUMERATE(i, num, sum, {
                if (num != prev_num)
                {
                    if (prev_num * prev_seq > max_area)
                    {
                        max_area = prev_num * prev_seq;
                    }
                    prev_seq = 1;
                    prev_num = num;
                    continue;
            ...
            if (prev_seq != 0 && index == sum.size() - 1)
            {
                if (prev_seq * prev_num > max_area)
                {
                    max_area = prev_seq * prev_num;
                }
            }
            println(sum, " [", max_area, "]");
            grid[row] = ro;
        }
