...
map<char, ll> counts(const std::string &s)
{
    map<char, ll> m;
    for (const auto &ch : s)
    {
...
int method1()
{
    ...
    string newspaper = reads();

    ll yays = 0, whoops = 0;

    auto msg_counts = counts(message);
    auto n_counts = counts(newspaper);

    for (const auto &record : msg_counts)
    {
        // 1. First prioritize making a yay, then making a whoop
        if (n_counts[record.first] >= record.second)
        {
            // The newspaper has equal or more characters than is required.
            n_counts[record.first] -= record.second;
            yays += record.second;
        }
        else
        {
            // The newspaper has less characters than required, we need to
            // borrow some from the opposite case The newspaper has equal or
            // more characters than is required.
            yays += n_counts[record.first];
            ll required = record.second - n_counts[record.first];
            n_counts[record.first] = 0;

            char revcase = islower(record.first) ? toupper(record.first)
                                                 : tolower(record.first);
            whoops += required;
            if (n_counts[revcase] < required)
            {
                std::cerr << "Less than required chars\n";
            }
            n_counts[revcase] -= required;
        }
    }
    println(yays, " ", whoops);
    return 0;
...
int main()
{
    ...
    for (const auto &ch : message)
    {
        if(!isalpha(ch))
            continue;
        char revcase = islower(ch) ? toupper(ch) : tolower(ch);
        if (n_counts[revcase] > 0)
        {
            n_counts[revcase] -= 1;
            whoops += 1;
        }
