// ://github.com/ananthvk/codeforces-solutions
...
int main()
{
    ...
    ll m = readn();
    vector<string> grid;
    map<ll, map<char, ll>> row_counts;
    map<ll, map<char, ll>> col_counts;
    grid.reserve(n);
    FORN(i, n)
    {
        grid.push_back(reads());
    }
    FORN(i, n)
    {
        for (const auto &ch : grid[i])
        {
            row_counts[i][ch]++;
        }
    }
    ...
    FORN(i, n)
    {
        ...
        {
            char ch = grid[i][j];
            if (row_counts[i][ch] != 1)
                continue;
            if (col_counts[j][ch] != 1)
                continue;
            print(grid[i][j]);
        }
    }
...
// https://codeforces.com/contest/90/problem/B
