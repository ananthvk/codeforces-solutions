...
int main()
{
    ...
    FORN(nt, ntests)
    {
        ...
        ll half_point = std::ceil(g / 2.0);
        if ((half_point - 1) * n >= (k * g))
        {
            println(k * g);
        }
        else
        {
            ll base_savings = (half_point - 1) * n;
            ll remains = (k * g) - base_savings;

            ll quo = remains / g;
            ll rem = remains % g;

            // base_savings -= quo * (half_point-1);
            // println("[", base_savings, " ", remains, " ", quo, " ", rem);
            if (rem > 0)
            {
                if (rem + (half_point - 1) < g)
                {
                    rem + half_point
                }
                else
                {
                    base_savings += (half_point - 1 + rem) % g;
                    base_savings -= half_point - 1;
                }
            }
            // println("]", base_savings, " ", remains, " ", quo, " ", rem);
            println(base_savings);
        }
    }
    return 0;
}
