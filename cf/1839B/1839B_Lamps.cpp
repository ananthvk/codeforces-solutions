...
int main()
{
    FAST_IO
    // Pseudocode
    // while there are no lamps which are off
    //    Choose the best lamp, which is the lamp with least a_i and
    //    maximum b_i and is not broken
    //    Break all lamps which have value less than or equal to x
    ll ntests = readn();
    FORN(i, ntests){
        ll n = readn();
        FORN(i, n){
