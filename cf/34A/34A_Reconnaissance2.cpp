...
int main()
{
    ...
    ll n = readn();
    auto v = readvin(n);
    ll smallest_diff = MAXVAL(ll);
    pair<ll, ll> smallest;
    FORS(i, 0, v.size() - 1, 1)
    {
        ll diff = std::abs(v[i] - v[i + 1]);
        if (diff < smallest_diff)
        {
            smallest_diff = diff;
            smallest.first = i + 1;
            smallest.second = i + 1 + 1;
        }
    }
    if (std::abs(v.front() - v.back()) < smallest_diff)
    {
        smallest.first = 1;
        smallest.second = v.size();
    }
    println(smallest.first, " ", smallest.second);
    return 0;
}
