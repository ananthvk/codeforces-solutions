...
int main()
{
    ...
    // For maximizing the number of pairs, keep only one person in each team and put the remaining in one of the teams
    ll max_pairs = get_number_of_pairs(n - (m - 1));
    // To minimize the number of pairs, distribute everyone equally so that each team has the least possible members
    ll per_team = n / m;
    ll left_out = n % m;
    ll min_pairs = (left_out) * get_number_of_pairs(per_team + 1) + (m - left_out) * get_number_of_pairs(per_team);
    println(min_pairs, " ", max_pairs);
    return 0;
}
